...
const apiURL =
  ...
  export const newPost = async (newPost) => {
      try {
          ...
          if (token) {
              const config = buildHeaders(token);
              const response = await axios.post(`${apiURL}/posts/new-post`, newPost, config);
              return response.data;
          }
      } catch (error) {
          console.error(error.message);
...
export const updatePost = async (id) => {
    try {
        ...
        if (token) {
            ...
            const response = await axios.put(`${apiURL}/posts/update-post/${id}`, config);
            return response.data;
        }
    } catch (error) {
        console.error(error.message);
...
export const destroyPost = async (id) => {
    try {
        ...
        if (token) {
            const config = buildHeaders(token);
            const response = await axios.delete(`${apiURL}/posts/delete-post/${id}`, config);
            return response.data;
        }
    } catch (error) {
        console.error(error.message);
